<?php
// ============================================================
// RideEase – API: Submit Driver Rating & Feedback (Design user rating feedback submission forms, save feedback details & compute updated driver average star ratings)
// ============================================================
...
try {
    ...
    // Recalculate Driver Ratings stats (avg only)
    $statsStmt = $db->prepare("SELECT AVG(rating) as avg_r FROM ratings WHERE driver_id = ?");
    $statsStmt->execute([$ride['driver_id']]);
    $avgRating = $statsStmt->fetchColumn();

    $updateDriver = $db->prepare("UPDATE drivers SET avg_rating = ? WHERE id = ?");
    $updateDriver->execute([$avgRating, $ride['driver_id']]);
